...
{{< figure src="nixl-comm-stack.svg" alt="NIXL communication stack: six layers from inference engines down to hardware fabric" caption="The disaggregated serving communication stack showing how KV cache transfers flow through NIXL. Layer 1: Inference engines (TRT-LLM, vLLM, SGLang) manage KV in GPU VRAM on prefill and decode workers. Layer 2: Dynamo's `nixl_connect` Python wrapper provides an optional application interface — engines can call NIXL directly. Layer 3: NIXL core library handles registered memory segments and READ/WRITE operations. Layer 4: NIXL plugins (UCX, LIBFABRIC, GDS) provide transport abstraction. Layer 5: UCX Transport Layer Selections (TLS) — rc_x/dc_x for RDMA over InfiniBand/RoCE (fast path), cuda_ipc for same-node transfers, cuda_copy for host-staged transfers, tcp as fallback. Layer 6: Hardware interconnects — NVLink/NVSwitch for same-node multi-GPU, InfiniBand/RoCE/EFA for cross-node RDMA. Fast path avoids host staging." >}}
