...
{{< figure src="nixl-comm-stack.svg" alt="NIXL communication stack: six layers from inference engines down to hardware fabric" caption="The disaggregated serving communication stack showing how KV cache transfers flow through NIXL. Layer 1: Inference engines (TRT-LLM, vLLM, SGLang) manage KV in GPU VRAM on prefill and decode workers. Layer 2: Dynamo's `nixl_connect` Python wrapper provides the application interface. Layer 3: NIXL core library handles registered memory segments and READ/WRITE operations. Layer 4: NIXL plugins (UCX, LIBFABRIC, GDS) provide transport abstraction. Layer 5: UCX Transport Layer Selections (TLS) — rc_x/dc_x for RDMA over InfiniBand/RoCE, cuda_ipc for same-node transfers, cuda_copy for host-staged transfers, tcp as fallback. Layer 6: Hardware interconnects — NVLink/NVSwitch for same-node multi-GPU, InfiniBand/RoCE/EFA for cross-node RDMA. All KV transfers go through the full stack; NIXL is the unified path, not bypassed." >}}

The diagram above shows where NIXL fits in Dynamo's disaggregated serving stack. At the bottom (Layer 6) sits the hardware fabric: NVLink/NVSwitch for same-node multi-GPU communication, or InfiniBand/RoCE/EFA for cross-node RDMA. Layer 5 contains UCX's transport layer selections (TLS) — rc_x/dc_x for RDMA, cuda_ipc for same-node IPC, cuda_copy for host-staged paths, and tcp as a fallback. Layer 4 contains the NIXL plugins (UCX, LIBFABRIC, GDS) that abstract these transports. NIXL core (Layer 3) sits above, managing registered memory segments and providing READ/WRITE operations. Dynamo's `nixl_connect` wrapper (Layer 2) provides the Python interface, and at the top (Layer 1) are the inference framework backends (TensorRT-LLM, vLLM, SGLang) that trigger transfers when they need to move KV cache from a prefill worker to a decode worker.
