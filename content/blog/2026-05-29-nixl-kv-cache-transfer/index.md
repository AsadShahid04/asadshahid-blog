...
{{< figure src="nixl-comm-stack.svg" alt="NIXL communication stack: six layers from inference engines down to hardware fabric" caption="The disaggregated serving communication stack. Layer 1: Inference engines (TRT-LLM, vLLM, SGLang) manage KV in GPU VRAM on prefill and decode workers. Layer 2: Dynamo's `nixl_connect` is the optional Dynamo Python path — vLLM NixlConnector and TRT-LLM NIXL can skip this layer and call NIXL core directly. Layer 3: NIXL core library handles registered memory segments and READ/WRITE operations. Layer 4: NIXL plugins (UCX, LIBFABRIC, GDS) provide transport abstraction. Layer 5 branches by plugin: UCX plugin uses UCX TLS (rc_x/dc_x for RDMA, cuda_ipc, cuda_copy, tcp); LIBFABRIC plugin uses EFA and libfabric fabrics; GDS plugin uses GPUDirect Storage for direct GPU I/O. Layer 6 branches by plugin: UCX uses InfiniBand/RoCE/NVLink; LIBFABRIC uses EFA; GDS uses NVMe/GPUDirect Storage (local storage I/O, not network fabric)." >}}

The diagram above shows where NIXL fits in Dynamo's disaggregated serving stack. At the bottom (Layer 6) the hardware path branches by plugin: when using the UCX plugin, it accesses InfiniBand/RoCE for cross-node RDMA or NVLink for same-node transfers; when using the LIBFABRIC plugin, it accesses EFA (AWS Elastic Fabric Adapter); when using GDS, it accesses NVMe and GPUDirect Storage for local storage I/O (not network fabric). Layer 5 branches by plugin: when using the UCX plugin, it accesses UCX transport layer selections (TLS) — rc_x/dc_x for RDMA (the fast path), cuda_ipc for same-node IPC, cuda_copy for host-staged paths, and tcp as fallback; when using the LIBFABRIC plugin, it accesses EFA and other libfabric fabrics; when using GDS, it accesses GPUDirect Storage for direct GPU I/O. Layer 4 contains the NIXL plugins (UCX, LIBFABRIC, GDS) that abstract these transports. NIXL core (Layer 3) sits above, managing registered memory segments and providing READ/WRITE operations. Dynamo's `nixl_connect` wrapper (Layer 2) is an optional Dynamo Python path — vLLM NixlConnector and TRT-LLM NIXL can skip this layer and call NIXL core directly. At the top (Layer 1) are the inference framework backends (TensorRT-LLM, vLLM, SGLang) that trigger transfers when they need to move KV cache from a prefill worker to a decode worker.
...
{{< figure src="nixl-cross-node.svg" alt="Cross-node KV cache transfer: GPU VRAM to GPU VRAM via NIXL RDMA WRITE fast path, with host staging as slow path" caption="Cross-node disaggregated serving: the fast path transfers KV cache buffers directly from prefill node GPU VRAM to decode node GPU VRAM via NIXL RDMA WRITE over InfiniBand, RoCE, or EFA. Host DRAM is off the RDMA path. The dashed red path shows the slow route through host staging (cuda_copy): GPU → Host → Network → Host → GPU, which still exists but NIXL aims to avoid." >}}
