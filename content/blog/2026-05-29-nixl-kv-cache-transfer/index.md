...
The diagram above shows where NIXL fits in Dynamo's disaggregated serving stack. At the bottom (Layer 6) sits the hardware fabric: NVLink/NVSwitch for same-node multi-GPU communication, or InfiniBand/RoCE/EFA for cross-node RDMA. Layer 5 contains UCX's transport layer selections (TLS) — rc_x/dc_x for RDMA (the fast path), cuda_ipc for same-node IPC, cuda_copy for host-staged paths, and tcp as a fallback. Layer 4 contains the NIXL plugins (UCX, LIBFABRIC, GDS) that abstract these transports. NIXL core (Layer 3) sits above, managing registered memory segments and providing READ/WRITE operations. Dynamo's `nixl_connect` wrapper (Layer 2) provides an optional Python interface for Dynamo-based deployments — inference engines can also call NIXL directly without this layer. At the top (Layer 1) are the inference framework backends (TensorRT-LLM, vLLM, SGLang) that trigger transfers when they need to move KV cache from a prefill worker to a decode worker.
...
{{< figure src="nixl-cross-node.svg" alt="Cross-node KV cache transfer: GPU VRAM to GPU VRAM via NIXL RDMA WRITE fast path, with host staging as slow path" caption="Cross-node disaggregated serving: the fast path transfers KV cache buffers directly from prefill node GPU VRAM to decode node GPU VRAM via NIXL RDMA WRITE over InfiniBand, RoCE, or EFA. Host DRAM is not on the fast path. The dashed red path shows the slow route through host staging (cuda_copy): GPU → Host → Network → Host → GPU, which NIXL aims to avoid." >}}
